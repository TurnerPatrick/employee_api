...
def add_to_env(length):
    """
        ...
        that will live inside of the .env file when script
        is run
    ...
    with open('.env', 'w+') as file:
        database_url = 'postgresql+psycopg2://{user}:{pw}@{url}/{db}'.format(user='postgres',pw='winded07',url='127.0.0.1:5432',db='employee_api_db')
        data = file.read()
        file.seek(0)
